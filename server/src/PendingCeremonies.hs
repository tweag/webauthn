...
-- | Stores a new pending ceremony in memory along with its 'M.CredentialOptions' options.
-- The challenge to be used for the options is generated by this function.
-- This function can be used for both register and login webauthn ceremonies
insertPendingCeremony ::
  forall c.
  SingI c =>
  PendingCeremonies ->
...
insertPendingCeremony pendingOps = case sing @c of
  WA.SRegistration -> insert (pendingRegisters pendingOps)
  WA.SAuthentication -> insert (pendingLogins pendingOps)
  where
    insert ::
      Pendings c ->
      (WA.Challenge -> WA.CredentialOptions c) ->
...
-- | Gets a pending ceremony in memory along with its 'M.CredentialOptions',
-- given a 'M.Credential' which contains the challenge previously generated by 'insertPendingCeremonies'.
...
getPendingCeremony pending cred = case sing @c of
  -- We extract the challenge from the response credential that was sent back
  WA.SRegistration -> get (pendingRegisters pending) (WA.ccdChallenge $ WA.arrClientData $ WA.cResponse cred)
  WA.SAuthentication -> get (pendingLogins pending) (WA.ccdChallenge $ WA.araClientData $ WA.cResponse cred)
  where
    get ::
      Pendings c ->
      WA.Challenge ->
...
-- The result can be used with functions 'insertPendingCeremony' and 'getPendingCeremony'
newPendingCeremonies ::
  PendingCeremoniesConfig ->
