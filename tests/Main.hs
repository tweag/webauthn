...
import qualified Crypto.WebAuthn.Encoding.WebAuthnJson as M
...
registerTestFromFile fp origin rpId verifiable service now = do
  pkCredential <-
    either (error . show) id . M.wjDecodeCredentialRegistration
      <$> decodeFile fp
  ...
  describe "RegisterAndLogin" $
    it "tests whether the fixed register and login responses are matching" $
      do
        ...
        loginReq <-
          either (error . show) id . M.wjDecodeCredentialAuthentication
            <$> decodeFile
              @M.WJCredentialAuthentication
